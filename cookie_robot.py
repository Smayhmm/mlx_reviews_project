            ...
            try:
                ...
                while not profile_started:
                    (
                        ...
                        profile_started,
                        message,
                    ) = self.mlx.start_quick_profile(self.browser_type)
                    if profile_started:
                        return
                    print(
                        f"Profile couldn't be started. Probably downloading core. Will wait for 60 seconds and try again. Here is the message: {message}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--guided", action="store_true", help="Run the script in guided mode"
    )
    ...
    if guided_mode:
        ...
        BROWSER = (
            input("What is the browser type? Stealthfox or Mimic: ").strip().lower()
        )
    ...
    # If assistance is not needed, retrieve values from .env file
        ...
        EMAIL = Credentials.MLX_EMAIL 
        PASSWORD = Credentials.MLX_PASSWORD 
        BROWSER = "mimic"
        TYPE = "normal"
        PROFILE_ID = os.getenv('PROFILEID')
        # PROFILE_ID = "09400797-4834-4eeb-a3dd-bf9123bf4412"
